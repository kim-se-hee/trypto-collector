...
**의존성:** 각 거래소 RestClient, MarketInfoCache, RealtimePriceCollector, TickerRedisRepository, MarketMetadataRedisRepository

**초기화 흐름:** `init()`에서 `loadUpbit()`, `loadBithumb()`, `loadBinance()`를 각각 `subscribe()`로 실행한다.

| 메서드 | 흐름 |
|--------|------|
| `loadUpbit()` | REST 호출 → 캐시 적재 → Redis 메타데이터 저장 → `connectUpbit()` |
| `loadBithumb()` | REST 호출 → 캐시 적재 → Redis 메타데이터 저장 → `connectBithumb()` |
| `loadBinance()` | REST 호출 → 캐시 적재 + Redis 초기 스냅샷 → Redis 메타데이터 저장 → `connectBinance()` |
...
`save(Exchange, List<MarketInfo>)` 메서드 하나만 제공한다. `Mono<Boolean>`을 반환한다.
