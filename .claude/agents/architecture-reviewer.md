...
```
ksh.tryptocollector/
  common/config/        설정 클래스 (WebClient, WebSocket)
  common/model/         공통 모델 (Exchange, NormalizedTicker, TickerEvent)
...
**데이터 흐름 방향:**
```
client/rest → metadata → client/websocket → common/model → redis + rabbitmq
                                                  ↑
                                              collector (오케스트레이션)
```
...
- [ ] **하류 → 상류 역의존**: `redis`나 `rabbitmq` 패키지가 `client/websocket`이나 `collector`를 import (저장소가 수집기를 알면 안 됨)
- [ ] **common → 기능 패키지 의존**: `common/model`이나 `common/config`가 `redis`, `rabbitmq`, `client` 등 기능 패키지를 import
- [ ] **client/rest ↔ client/websocket 교차 의존**: REST 클라이언트와 WebSocket 핸들러가 서로를 직접 참조
- [ ] **순환 의존**: 패키지 A → B → A 형태의 순환 참조
- [ ] **metadata → client/websocket 의존**: 메타데이터 캐시가 WebSocket 핸들러를 직접 알아서는 안 됨 (초기화 흐름은 `ExchangeInitializer`가 오케스트레이션)
...
- [ ] **오케스트레이터에 세부 구현**: `collector` 패키지가 정규화, 직렬화 등 세부 로직을 직접 구현 (위임해야 함)
...
- [ ] **DTO 위치 오류**: REST DTO가 `client/rest/dto/`가 아닌 곳에 위치하거나, WebSocket DTO가 `client/websocket/dto/`가 아닌 곳에 위치
- [ ] **DTO에 Spring 의존**: DTO(`record`)가 Spring 어노테이션(`@Component`, `@Service` 등)에 의존
- [ ] **NormalizedTicker 외부 노출**: 거래소별 DTO가 `common/model` 밖으로 직접 전달 (정규화 후에는 `NormalizedTicker`만 사용해야 함)
